...
void fitWe(const TString  outputDir="test",  // output directory
					 ...
					 const Int_t    plotOption=2,      // Set Options below
	...
	Float_t weight,puweight,pt,eta,iso;
	...
	for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
		...
		intree->SetBranchAddress("leg1_relIso", &iso);
		...
		for(UInt_t ientry=0; ientry<intree->GetEntries(); ientry++) {
			...
			if(!doMu && fabs(cat) != 11)           continue;
			if( doMu && fabs(cat) != 13)           continue;
			if(pt        < PT_CUT)  continue;
			if(fabs(eta) > ETA_CUT) continue;
			if( (typev[ifile]==eData     || typev[ifile]==eW     || typev[ifile]==eEWK    ) && ((iso > 0.12 && (fabs(cat) == 13)) || (iso > 0.10 && fabs(cat) == 11) ))  continue;
			if( (typev[ifile]==eAntiData || typev[ifile]==eAntiW || typev[ifile]==eAntiEWK) && (iso < 0.30 || iso > 0.50) )  continue;
			puweight *= weight;
			total += puweight;
			npass ++;
